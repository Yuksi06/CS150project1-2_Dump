...
class ServerGameState implements Updatable {
  ...
  // 5 Slots Total
  final List<bool> _reservedSlots = [false, false, false, false, false];

  final double initialDuration;
  double timeRemaining; // USAGE: In Lobby = PlayerLimit. In Game = Seconds.
  ...
  ServerGameState({required double duration}) 
    : timeRemaining = duration, // Will be overwritten by setPlayerLimit immediately
      initialDuration = duration {
    _generateMap();
  }
  
  void setPlayerLimit(int limit) {
    totalPlayersRequired = limit;
    if (isLobby) {
      timeRemaining = limit.toDouble();
    }
  }
  
  int reserveNextSlot() {
    // Only allow reservation if we haven't reached the limit
    int currentCount = 0;
    for(bool b in _reservedSlots) if(b) currentCount++;
    
    if (currentCount >= totalPlayersRequired) return -1; // Strict Limit
  ...
  bool get isFull {
     int count = 0;
     for(bool b in _reservedSlots) if(b) count++;
     return count >= totalPlayersRequired;
  }
  ...
  void _generateMap() {
    ...
    for(int y = 0; y < gameHeight; y++) {
      for (int x = 0; x < gameWidth; x++) {
        ...
        if ((x == 7 && y == 6) || (x == 8 && y == 6) || (x == 6 && y == 6) || (x == 7 && y == 7) || (x == 7 && y == 5)) continue;

        if (rng.nextDouble() < 0.4) _softBlocks.add(y * gameWidth + x);
      }
    }
  }
  ...
  void addEntity(GameEntity entity) => _pendingAdditions.add(entity);

  PlayerEntity? getPlayer(int id) {
    try { return _entities.whereType<PlayerEntity>().firstWhere((e) => e.id == id); } 
    catch (e) { try { return _pendingAdditions.whereType<PlayerEntity>().firstWhere((e) => e.id == id); } catch (e2) { return null; } }
  }

  List<PlayerEntity> getPlayers() {
    return [..._entities.whereType<PlayerEntity>(), ..._pendingAdditions.whereType<PlayerEntity>()];
  }

  BombEntity? getBombAt(int x, int y) {
    for (var entity in _entities) if (entity is BombEntity && entity.gridX == x && entity.gridY == y) return entity;
    for (var entity in _pendingAdditions) if (entity is BombEntity && entity.gridX == x && entity.gridY == y) return entity;
    return null;
  }

  void explode(int x, int y, int range) {
    List<List<int>> fireTiles = [[x, y]];
    void castRay(int dx, int dy) {
      for (int i = 1; i <= range; i++) {
        ...
        if (hitBomb != null) { if (hitBomb.timer > 0.1) hitBomb.timer = 0.05; fireTiles.add([tx, ty]); break; }
        ...
        for (var entity in _entities) {
          if (entity is PowerupEntity) { if (entity.x.toInt() == tx && entity.y.toInt() == ty) entity.shouldRemove = true; }
        }
      }
    }
    castRay(0, -1); castRay(0, 1); castRay(-1, 0); castRay(1, 0);
  ...
  void update(double dt) {
    ...
    if (isLobby) {
        if (timeRemaining != totalPlayersRequired.toDouble()) {
           timeRemaining = totalPlayersRequired.toDouble();
        }

        int connectedCount = getPlayers().length;
        
        if (connectedCount >= totalPlayersRequired) {
           bool allReady = getPlayers().every((p) => p.isReady);
           if (allReady && !isCountdown) {
              startCountdown();
           }
        }
        return;
    ...
    if (timeRemaining <= 0 && currentWinnerId == null) {
      var alive = _entities.whereType<PlayerEntity>().where((p) => !p.isDead).toList();
      currentWinnerId = alive.length == 1 ? alive.first.id : -1;
    }

    for (var entity in _entities) entity.update(dt);
    if (_pendingAdditions.isNotEmpty) { _entities.addAll(_pendingAdditions); _pendingAdditions.clear(); }
    _entities.removeWhere((e) => e.shouldRemove);

    var alive = _entities.whereType<PlayerEntity>().where((p) => !p.isDead).toList();
    if (_entities.whereType<PlayerEntity>().length > 1) {
      if (alive.length <= 1) {
         if (gameEndDelayTimer == -1) gameEndDelayTimer = 1.0; 
         else {
            gameEndDelayTimer -= dt;
            if (gameEndDelayTimer <= 0) {
               currentWinnerId = alive.length == 1 ? alive.first.id : (alive.isEmpty ? -1 : null);
            }
         }
      } else { gameEndDelayTimer = -1; }
    }

    if (currentWinnerId != null) {
      restartTimer += dt;
      if (restartTimer > 5.0) _resetGame();
    }
  }
  ...
  void _resetGame() {
    ...
    currentWinnerId = null; restartTimer = 0; gameEndDelayTimer = -1; 
    _softBlocks.clear(); _pendingAdditions.clear();
    _entities.removeWhere((e) => e is! PlayerEntity);

    for (var entity in _entities) {
      if (entity is PlayerEntity) {
        entity.isDead = false; entity.shouldRemove = false; entity.speed = 120.0; 
        entity.explosionRange = 1; entity.maxBombs = 1; entity.isReady = false; 
        entity.isInvincible = false; entity.invincibleTimer = 0.0;
        ...
        else if (entity.id == 4) { entity.x = 7.0 * 32.0; entity.y = 6.0 * 32.0; }
    ...
    // Set timeRemaining back to Limit so clients know slots
    timeRemaining = totalPlayersRequired.toDouble(); 
  ...
  GameStateModel getSnapshot() {
    List<PlayerModel> players = [..._entities.whereType<PlayerEntity>(), ..._pendingAdditions.whereType<PlayerEntity>()].map((p) => p.toModel()).toList();
    List<BombModel> bombs = _entities.whereType<BombEntity>().map((b) => b.toModel()).toList();
    List<List<int>> blocks = _softBlocks.map((encoded) => [encoded % gameWidth, encoded ~/ gameWidth]).toList();
    List<ExplosionModel> explosions = _entities.whereType<ExplosionEntity>().map((e) => e.toModel()).toList();
    List<PowerupModel> powerups = _entities.whereType<PowerupEntity>().map((e) => e.toModel()).toList();
    
    return GameStateModel(players: players, softBlocks: blocks, bombs: bombs, explosions: explosions, powerups: powerups, timeRemaining: timeRemaining, winnerId: currentWinnerId);
  }
}
...
class PowerupEntity extends GameEntity {
  ...
  @override void update(double dt) {
    for (var player in state.getPlayers()) {
      ...
      int px = (player.x / 32 + 0.5).floor(); int py = (player.y / 32 + 0.5).floor();
      if (px == x && py == y) {
        if (type == 0) player.explosionRange++; else if (type == 1) { if (player.speed < 250.0) player.speed += 40.0; } else if (type == 2) player.maxBombs++; else if (type == 3) { player.isInvincible = true; player.invincibleTimer = 10.0; }
        shouldRemove = true;
      }
    }
  }
