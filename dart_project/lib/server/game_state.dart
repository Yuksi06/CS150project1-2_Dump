...
class ServerGameState implements Updatable {
  ...
  BombEntity? getBombAt(int x, int y) {
    for (var entity in _entities) {
      if (entity is BombEntity && entity.gridX == x && entity.gridY == y) return entity;
    }
      ...
      for (int i = 1; i <= range; i++) {
        ...
        for (var entity in _entities) {
          if (entity is PowerupEntity) {
            if (entity.x.toInt() == tx && entity.y.toInt() == ty) entity.shouldRemove = true;
          }
        }
      }
    }
  ...
  void _resetGame() {
    ...
    for (var entity in _entities) {
      if (entity is PlayerEntity) {
        ...
        entity.invincibleTimer = 0.0;

        if (entity.id == 0) {
          entity.x = 1.0 * 32.0;
          entity.y = 1.0 * 32.0;
        } else if (entity.id == 1) {
          entity.x = (gameWidth - 2.0) * 32.0;
          entity.y = (gameHeight - 2.0) * 32.0;
        } else if (entity.id == 2) {
          entity.x = (gameWidth - 2.0) * 32.0;
          entity.y = 1.0 * 32.0;
        } else if (entity.id == 3) {
  ...
  GameStateModel getSnapshot() {
    List<PlayerModel> players = [..._entities.whereType<PlayerEntity>(), ..._pendingAdditions.whereType<PlayerEntity>()]
        .map((p) => p.toModel())
        .toList();
...
class PlayerEntity extends GameEntity {
  ...
  int extraLives = 0; // Option 5: Heart
  ...
  bool _checkCollision(double newX, double newY) {
    ...
    if (hitTerrain(left, top) || hitTerrain(right, top) || hitTerrain(left, bottom) || hitTerrain(right, bottom)) {
      return true;
    }
    ...
    if (hitsBlockingBomb(left, top) ||
        hitsBlockingBomb(right, top) ||
        hitsBlockingBomb(left, bottom) ||
        hitsBlockingBomb(right, bottom)) {
      return true;
    }
    return false;
  }
  ...
  void update(double dt) {
    ...
    for (var player in state.getPlayers()) {
      ...
      for (var tile in tiles) {
        if (tile[0] == px && tile[1] == py) { // Option 5: Heart
